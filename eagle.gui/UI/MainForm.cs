using Eagle.Core;
using Eagle.Cryptography;
using Eagle.Implementations.Blockchains.LevelDB;
using Eagle.Implementations.Wallets.EntityFramework;
using Eagle.Implementations.Wallets.NEP6;
using Eagle.IO;
...
namespace Eagle.UI
{
    internal partial class MainForm : Form
    {
        ...
        private void ChangeWallet(Wallet wallet)
        {
            ...
            汽油萃取CToolStripMenuItem.Enabled = Program.CurrentWallet != null;
                ...
                if (Program.CurrentWallet.WalletHeight <= Blockchain.Default.Height + 1)
                {
                    if (balance_changed)
                    {
                        ...
                        foreach (var asset in assets.Values)
                        {
                            ...
                            {
                                string asset_name = asset.Asset.AssetType == AssetType.GoverningToken ? "EGX" :
                                                    asset.Asset.AssetType == AssetType.UtilityToken ? "BENZ" :
                                                    asset.Asset.GetName();
        ...
        private void 汽油萃取CToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        {
            Process.Start("https://eaglepay.io/");
        }
