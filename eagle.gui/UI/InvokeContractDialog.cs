...
namespace Eagle.UI
{
    internal partial class InvokeContractDialog : Form
    {
        ...
        private static readonly Fixed8 net_fee = Fixed8.FromDecimal(0.01m);
        ...
        {
            Fixed8 fee = tx.Benz.Equals(Fixed8.Zero) ? net_fee : Fixed8.Zero;
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            sb.AppendLine($"Benz Consumed: {engine.BenzConsumed}");
            sb.AppendLine($"Evaluation Stack: {new JArray(engine.EvaluationStack.Select(p => p.ToParameter().ToJson()))}");
            textBox7.Text = sb.ToString();
            if (!engine.State.HasFlag(VMState.FAULT))
            {
                tx.Benz = engine.BenzConsumed - Fixed8.FromDecimal(10);
                if (tx.Benz < Fixed8.Zero) tx.Benz = Fixed8.Zero;
                tx.Benz = tx.Benz.Ceiling();
                Fixed8 fee = tx.Benz.Equals(Fixed8.Zero) ? net_fee : tx.Benz;
                label7.Text = fee + " benz";
